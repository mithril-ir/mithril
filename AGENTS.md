...
- Haskell is the selected implementation language for the deterministic host tool (see `docs/compiler-architecture.md`). The host tool now implements the first frontend boundary — `mithril validate FILE`, JSON parsing plus structural validation against this schema — and nothing after it: resolution, typing, and normalization do not exist.
