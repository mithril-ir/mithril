...
One derived artifact is producible today: the human-readable security contract, rendered on demand by `mithril contract FILE` from the typed normalized Core. It is not stored in the repository as a build product — the byte-frozen copies under `test/fixtures/` are reviewed test expectations that pin the renderer's exact output, and correcting a wrong contract means correcting the authored model or the renderer, never editing rendered output. Nothing else is generated: in particular, the `Mithril.Acme` proof slice is hand-transcribed authored Agda ([`agda/README.md`](../agda/README.md)); it is not derived from [`examples/acme/acme.mir.json`](../examples/acme/acme.mir.json), and no tool checks the transcription against the JSON.
...
As of 2026-08-20, exactly the four frontend boundaries of the pipeline above — deterministic JSON parsing plus Core v0 structural validation, complete Core v0 name resolution, complete Core v0 static typing, and deterministic Core v0 normalization, exposed together as `mithril validate FILE` — plus the contract renderer, exposed as `mithril contract FILE`, are implemented. Everything else after the frontend remains unbuilt.
...
- any verifier: no policy evaluation, no semantic diff, no proof checking from Core, and no guarantee checking — a normalized document's guarantees are selected proof obligations, established by nothing, and the rendered contract does not change that;
- any automated JSON-to-Agda connection — the `Mithril.Acme` slice is hand-transcribed, not derived from the JSON example;
- any Wasp generation, other target generation, or runtime enforcement — the contract renderer is the only consumer of the typed normalized Core so far, and it generates nothing executable;
- any generation metadata or manifest; the rendered contract is the only derived artifact, and it is produced on demand, not stored in the repository (the byte-frozen copies under `test/fixtures/` are test expectations, not distributed artifacts).

## Explicit non-claims

This document makes design commitments, not correctness claims:

- It does not claim that any pipeline stage beyond the four frontend boundaries — parsing, structural validation, name resolution, static typing, and normalization — and the contract renderer is implemented. None is: no Agda backend, no target emitter, and no verifier — no policy evaluator, no semantic diff, no proof checking from Core, and no runtime enforcement.
