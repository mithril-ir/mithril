...
The `Mithril.Core.Resolution` traversal is complete for the current Core v0 schema, but keeping schema and resolver evolution aligned is a manually reviewed contract. Template Haskell's `addDependentFile` registration and the resulting recompilation detect that the bytes of `core/schema.json` changed; they do not establish resolver completeness. A schema change is therefore not complete merely because structural validation and compilation pass.

Any change to `core/schema.json` that affects declaration namespaces; reference-bearing fields; term, policy, effect, result, or guarantee constructors; enum-value uniqueness or order assumptions; the distinguished `User` entity requirement; or any structural invariant on which resolution relies for totality or no-cascade behavior must include an explicit audit of `Mithril.Core.Resolution` and the relevant targeted regression tests in the same change. The audit must identify every added, removed, or changed reference location and demonstrate that resolution either traverses it or deliberately does not treat it as a name. Add positive, unknown-reference, duplicate/ambiguity, no-cascade, and invariant-drift regressions wherever the schema change makes them relevant. This audit is mandatory review-and-test discipline, not a currently automated completeness check; do not introduce a second copy of the schema or claim that recompilation automates it.
...
One representation decision is recorded as deliberately temporary. A `Resolved` document currently attests successful name resolution while retaining the structurally validated JSON value unchanged; that is an accepted checkpoint for this milestone, not the permanent representation later compiler stages consume. Before the typechecker is implemented, the frontend must introduce an explicit internal decoded/resolved Core representation, and the typechecker must be implemented over that representation — not directly over generic Aeson `Value`. The normalizer, the Agda emitter, and the backend adapters must likewise consume the frontend's representations rather than reinterpreting raw JSON or rebuilding name resolution independently (the per-backend half of this rule is already fixed under [One frontend, one normalized Core](#one-frontend-one-normalized-core)). No such internal representation exists today.
...
- the remaining semantic frontend stages: no Mithril typechecker and no normalizer — a resolved document is untyped and unnormalized, and no typed normalized Core exists;
- the explicit internal decoded/resolved Core representation described above — the prerequisite for starting the typechecker;
