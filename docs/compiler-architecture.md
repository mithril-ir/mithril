...
The architecture below is agreed and largely unbuilt. Exactly three frontend boundaries exist — deterministic JSON parsing plus Core v0 structural validation, complete Core v0 name resolution, and complete Core v0 static typing, exposed together as `mithril validate FILE`; [Current implementation status](#current-implementation-status) states exactly what exists today.
...
The frontend's one interpretation of the current Core v0 schema is complete — the internal schema decoder that turns structurally valid JSON into the explicit surface syntax, the decoded and resolved representation types that mirror every construct, the `Mithril.Core.Resolution` pass over them, and the static-typing pass (`Mithril.Core.Internal.Typecheck`, exposed through `Mithril.Core.Typing`) over the resolved model — but keeping schema and frontend evolution aligned is a manually reviewed contract. Template Haskell's `addDependentFile` registration and the resulting recompilation detect that the bytes of `core/schema.json` changed; they do not establish decoder, representation, resolver, or typechecker completeness. A schema change is therefore not complete merely because structural validation and compilation pass.

Any change to `core/schema.json` that affects declaration namespaces; reference-bearing fields; term, policy, effect, result, or guarantee constructors; enum-value uniqueness or order assumptions; the distinguished `User` entity requirement; or any structural invariant on which the frontend relies for totality or no-cascade behavior must include an explicit audit of the schema decoder and the representation's construct coverage as well as `Mithril.Core.Resolution` and the static typechecker, plus the relevant targeted regression tests, in the same change. The audit must identify every added, removed, or changed constructor and reference location and demonstrate that the decoder and representation carry it, that resolution either traverses it or deliberately does not treat it as a name, and that the typechecker either assigns it a typing judgment or deliberately has nothing to check for it (the typechecker's module documentation and the typing test module's audit table record that decision per construct). Add positive, unknown-reference, duplicate/ambiguity, well-typed/ill-typed, no-cascade, and invariant-drift regressions wherever the schema change makes them relevant. This audit is mandatory review-and-test discipline, not a currently automated completeness check; do not introduce a second copy of the schema or claim that recompilation automates it.
...
As of 2026-08-17, exactly three frontend boundaries of the pipeline above are implemented: deterministic JSON parsing plus Core v0 structural validation, complete Core v0 name resolution, and complete Core v0 static typing, exposed together as `mithril validate FILE`. Everything after them remains unbuilt.
...
- [`examples/acme/acme.mir.json`](../examples/acme/acme.mir.json) — a handwritten, unverified example model, which passes structural validation, name resolution, and static typing;
...
- the remaining semantic frontend stage: no normalizer — a typed document is unnormalized, and no typed *normalized* Core exists (the typed document above is the normalizer's intended input, not its output);
- any verifier: no policy evaluation and no guarantee checking — a well-typed document's guarantees are selected proof obligations, established by nothing;
...
- It does not claim that any pipeline stage beyond parsing, structural validation, name resolution, and static typing is implemented. None is.
