...
The fourth command runs the downstream API-boundary compile-fail probes: a probe package that depends on `mithril-ir` as an ordinary external package and proves that importing the hidden document module, using the `CoreDocument` constructor, and coercing between pipeline stages (or from a raw JSON value into one) all fail to compile for the intended reasons. It needs a POSIX shell in addition to the pinned toolchain.
...
One boundary of the resolved stage is deliberate and temporary: a successfully resolved document is represented as the unchanged, structurally validated JSON value carrying a name-resolution attestation — an accepted frontend checkpoint, not the permanent representation later compiler stages will consume. Before the typechecker is implemented, the frontend must introduce an explicit internal decoded/resolved Core representation; the typechecker, normalizer, Agda emitter, and backend adapters must not be built directly over generic raw JSON and must not reinterpret it or rebuild name resolution independently. That internal representation does not exist yet.
