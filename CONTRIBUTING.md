...
CI runs all of them on every push and pull request. Run them locally before submitting a change that touches the Haskell tool, `core/schema.json`, or `examples/acme/acme.mir.json`; the validate command must keep reporting the Acme example as valid Mithril Core v0 through normalization, and the contract command must keep rendering its frozen security contract (the exact bytes are pinned by the test suite's golden files under `test/fixtures/`). The probe script builds the expected-to-fail downstream package under `test/api-probes/` with a dedicated project file (`cabal.project.probes`) and verifies that every attack on the stage abstraction — importing the hidden document, representation, typechecker, normalized-representation, normalizer, or contract-renderer modules, calling the `CoreDocument` constructor (including to forge a `Typed` or `Normalized` document), coercing between stages (including from `Resolved` to `Typed` and from `Typed` to `Normalized`) or from a raw JSON value into a stage, coercing a resolved document back to a raw JSON value, or passing a merely `Typed` document to the public contract renderer — still fails to compile for its intended reason; because the internal representation, typechecker, normalizer, and contract-renderer modules cannot even be imported, downstream code can neither construct resolved or normalized nodes, typed or normalized documents, nor mint or cross-coerce internal identifiers. The compiling control exercises the legitimate pipeline through normalization and the public contract API. The same script then builds the flag-gated in-package probes of `mithril-ir` itself: after their control proves the real internal identifier definitions are in scope, the attack executables attempt cross-namespace `coerce` between global identifier types and must fail with GHC's representation-mismatch error — the identifier namespaces are distinct data types, non-coercible even for code inside the package. It needs a POSIX shell in addition to the pinned toolchain (on platforms without one, CI remains the authoritative runner).
