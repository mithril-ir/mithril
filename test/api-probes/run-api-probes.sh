...
expect probe-coerce-normalized \
  "match type .{1,3}Typed.{1,3} with .{1,3}Normalized" \
  "arising from a use of .{1,3}coerce"

expect probe-forge-normalized \
  "Illegal term-level use of the type constructor .{1,3}CoreDocument"

expect probe-hidden-normalized \
  "Could not load module" \
  "Mithril\.Core\.Internal\.Normalized" \
  "member of the hidden package .{1,3}mithril-ir-[0-9.]+:core-internal"

# The [^d] guard keeps this pattern from matching the *Normalized*
# representation module of the probe above: the pass module's name
# ends at "Normalize", so the next character in the diagnostic is
# GHC's closing quote, never a "d".
