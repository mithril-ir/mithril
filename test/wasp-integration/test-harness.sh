...
assert_contains "the same-operation denied-demotion counterexample (demote 403, keep 200, final top) is rejected" "$oracle_out" "^ok: same-operation demote = 403, keep = 200, final actor rank top (denied demotion beside a successful equal write): rejected$"
assert_contains "the cross-operation denied-demotion counterexample (rule 1 403, rule 2 200, final top) is rejected" "$oracle_out" "^ok: cross-operation rule 1 = 403, rule 2 = 200, final peer rank top (denied demotion beside a successful equal write): rejected$"
